...
class SaveRecoleccionEntrega extends StatefulWidget {
  const SaveRecoleccionEntrega({super.key});

  // ignore: constant_identifier_names
  static const String ROUTE = "/pagesave";

  @override
  State<SaveRecoleccionEntrega> createState() => _SavePageState();
}

class _SavePageState extends State<SaveRecoleccionEntrega> {
  ...
  Future<RecoleccionEntrega>? _futureRecoleccion;
  ...
  Widget build(BuildContext context) {
    ...
    RecoleccionEntrega? recoleccion = arguments['recoleccion'];
  ...
  _init(RecoleccionEntrega recoleccion) {
  ...
  Widget _buildForm(RecoleccionEntrega recoleccion) {
    _init(recoleccion);
                            ...
                            if (_formKey.currentState!.validate()) {
                              if (await Respuesta('Pregunta',
                                ...
                                if (recoleccion.id > 0) {
                                  ...
                                  setState(() {
                                    _futureRecoleccion =
                                        grabarRecoleccionEntrega(recoleccion);
                                  });
                                } else {
                                  setState(() {
                                    _futureRecoleccion =
  ...
  FutureBuilder<RecoleccionEntrega> buildRecoleccionFutureBuilder() {
    return FutureBuilder<RecoleccionEntrega>(
  ...
  Future<RecoleccionEntrega> grabarRecoleccionEntrega(
      RecoleccionEntrega recoleccion) async {
    bool dato = false;
    if (recoleccion.id > 0) {
      dato = await updateRecoleccionEntrega(recoleccion);
      // Operation.instance.updateRecoleccion(recoleccion);
      if (!dato) {
        throw Exception('Failed to update recoleccion.');
      }
    } else {
      dato = await createRecoleccionEntrega(recoleccion);
